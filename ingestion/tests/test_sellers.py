"""Unit and end-to-end tests for mercury_ingestion.connectors.sellers."""
...
from mercury_ingestion.connectors.sellers import REQUIRED_COLUMNS, SellersConnector
...
def _valid_sellers_csv(tmp_path: Path, name: str = "sellers.csv") -> Path:
...
class TestConstruction:
    def test_source_system_is_marketplace_platform(self, tmp_path: Path) -> None:
        connector = _make_connector(tmp_path, _valid_sellers_csv(tmp_path))
        assert connector.source_system == "marketplace_platform"

    def test_source_object_is_sellers(self, tmp_path: Path) -> None:
        connector = _make_connector(tmp_path, _valid_sellers_csv(tmp_path))
        assert connector.source_object == "sellers"

    def test_default_schema_version_is_1_0(self, tmp_path: Path) -> None:
        connector = SellersConnector(
            source_file=_valid_sellers_csv(tmp_path),
    ...
    def test_custom_schema_version_is_preserved(self, tmp_path: Path) -> None:
        connector = _make_connector(tmp_path, _valid_sellers_csv(tmp_path), schema_version="2.0")
...
class TestValidateSource:
    ...
    def test_required_columns_in_different_order(self, tmp_path: Path) -> None:
        ...
        source_file = _write_csv(tmp_path / "sellers.csv", shuffled_header, shuffled_rows)

        connector = _make_connector(tmp_path, source_file)
        connector.validate_source()  # should not raise

    def test_extra_columns_are_accepted(self, tmp_path: Path) -> None:
        header = [*HEADER, "seller_rating"]
        rows = [[*row, "4.8"] for row in SAMPLE_ROWS]
        source_file = _write_csv(tmp_path / "sellers.csv", header, rows)
    ...
    def test_non_csv_extension_produces_failed_metadata(self, tmp_path: Path) -> None:
        source_file = _write_csv(tmp_path / "sellers.txt", HEADER, SAMPLE_ROWS)
    ...
    def test_uppercase_csv_extension_is_accepted(self, tmp_path: Path) -> None:
        source_file = _write_csv(tmp_path / "sellers.CSV", HEADER, SAMPLE_ROWS)

        connector = _make_connector(tmp_path, source_file)
        connector.validate_source()  # should not raise

    def test_empty_file_produces_failed_metadata(self, tmp_path: Path) -> None:
        source_file = tmp_path / "sellers.csv"
    ...
    def test_invalid_utf8_produces_failed_metadata(self, tmp_path: Path) -> None:
        ...
        broken_header = b"seller_id,seller_zip_code_prefix,seller_city,seller_st\xffate\n"
        source_file.write_bytes(broken_header + b"s1,01310,sao paulo,SP\n")
...
class TestBusinessQualityBoundary:
    ...
    def test_blank_seller_state_is_accepted(self, tmp_path: Path) -> None:
        rows = [["s1", "01310", "sao paulo", ""]]
        source_file = _write_csv(tmp_path / "sellers.csv", HEADER, rows)
    ...
    def test_blank_seller_city_is_accepted(self, tmp_path: Path) -> None:
        rows = [["s1", "01310", "", "SP"]]
        source_file = _write_csv(tmp_path / "sellers.csv", HEADER, rows)
    ...
    def test_malformed_zip_code_prefix_is_accepted(self, tmp_path: Path) -> None:
        rows = [["s1", "not-a-zip!!", "sao paulo", "SP"]]
        source_file = _write_csv(tmp_path / "sellers.csv", HEADER, rows)
    ...
    def test_blank_zip_code_prefix_is_accepted(self, tmp_path: Path) -> None:
        rows = [["s1", "", "sao paulo", "SP"]]
        source_file = _write_csv(tmp_path / "sellers.csv", HEADER, rows)
    ...
    def test_duplicate_seller_id_values_are_accepted(self, tmp_path: Path) -> None:
        rows = [
            ["s1", "01310", "sao paulo", "SP"],
            ["s1", "20040", "rio de janeiro", "RJ"],
        ]
        source_file = _write_csv(tmp_path / "sellers.csv", HEADER, rows)
    ...
    def test_unusual_city_and_state_values_are_accepted(self, tmp_path: Path) -> None:
        rows = [["s1", "01310", "not_a_real_city_xyz", "ZZ"]]
        source_file = _write_csv(tmp_path / "sellers.csv", HEADER, rows)
...
class TestCountRecords:
    def test_excludes_header(self, tmp_path: Path) -> None:
        source_file = _write_csv(tmp_path / "sellers.csv", HEADER, SAMPLE_ROWS[:1])
    ...
    def test_header_only_csv_returns_zero(self, tmp_path: Path) -> None:
        source_file = _write_csv(tmp_path / "sellers.csv", HEADER, [])
    ...
    def test_quoted_fields_containing_commas_are_handled(self, tmp_path: Path) -> None:
        rows = [["s1", "01310", "Sao Paulo, capital", "SP"]]
        source_file = _write_csv(tmp_path / "sellers.csv", HEADER, rows)
...
class TestEndToEndLifecycle:
    ...
    def test_record_count_matches_number_of_seller_rows(self, tmp_path: Path) -> None:
        connector = _make_connector(tmp_path, _valid_sellers_csv(tmp_path))
    ...
    def test_schema_version_appears_in_completed_metadata(self, tmp_path: Path) -> None:
        connector = _make_connector(tmp_path, _valid_sellers_csv(tmp_path), schema_version="9.9")
    ...
    def test_validation_failure_does_not_land_a_raw_file(self, tmp_path: Path) -> None:
        header = [column for column in HEADER if column != "seller_state"]
