"""Unit and end-to-end tests for mercury_ingestion.connectors.payments."""
...
from mercury_ingestion.connectors.payments import REQUIRED_COLUMNS, PaymentsConnector
...
# Order "A" has two payment records (voucher + credit_card), order "B"
# has one -- the expected real-world shape where order_id repeats.
SAMPLE_ROWS = [
    ["A", "1", "voucher", "1", "20.00"],
    ["A", "2", "credit_card", "3", "80.00"],
    ["B", "1", "boleto", "1", "150.00"],
]
...
def _valid_payments_csv(tmp_path: Path, name: str = "payments.csv") -> Path:
...
class TestConstruction:
    def test_source_system_is_payment_platform(self, tmp_path: Path) -> None:
        connector = _make_connector(tmp_path, _valid_payments_csv(tmp_path))
        assert connector.source_system == "payment_platform"

    def test_source_object_is_payments(self, tmp_path: Path) -> None:
        connector = _make_connector(tmp_path, _valid_payments_csv(tmp_path))
        assert connector.source_object == "payments"

    def test_default_schema_version_is_1_0(self, tmp_path: Path) -> None:
        connector = PaymentsConnector(
            source_file=_valid_payments_csv(tmp_path),
    ...
    def test_custom_schema_version_is_preserved(self, tmp_path: Path) -> None:
        connector = _make_connector(tmp_path, _valid_payments_csv(tmp_path), schema_version="2.0")
...
class TestValidateSource:
    ...
    def test_required_columns_in_different_order(self, tmp_path: Path) -> None:
        ...
        source_file = _write_csv(tmp_path / "payments.csv", shuffled_header, shuffled_rows)

        connector = _make_connector(tmp_path, source_file)
        connector.validate_source()  # should not raise

    def test_extra_columns_are_accepted(self, tmp_path: Path) -> None:
        header = [*HEADER, "gateway_reference"]
        rows = [[*row, "ref-123"] for row in SAMPLE_ROWS]
        source_file = _write_csv(tmp_path / "payments.csv", header, rows)
    ...
    def test_non_csv_extension_produces_failed_metadata(self, tmp_path: Path) -> None:
        source_file = _write_csv(tmp_path / "payments.txt", HEADER, SAMPLE_ROWS)
    ...
    def test_uppercase_csv_extension_is_accepted(self, tmp_path: Path) -> None:
        source_file = _write_csv(tmp_path / "payments.CSV", HEADER, SAMPLE_ROWS)

        connector = _make_connector(tmp_path, source_file)
        connector.validate_source()  # should not raise

    def test_empty_file_produces_failed_metadata(self, tmp_path: Path) -> None:
        source_file = tmp_path / "payments.csv"
    ...
    def test_invalid_utf8_produces_failed_metadata(self, tmp_path: Path) -> None:
        ...
        broken_header = (
            b"order_id,payment_sequential,payment_type,payment_installments,"
            b"payment_val\xffue\n"
        )
        source_file.write_bytes(broken_header + b"A,1,voucher,1,20.00\n")
...
class TestBusinessQualityBoundary:
    ...
    def test_negative_payment_value_is_accepted(self, tmp_path: Path) -> None:
        rows = [["A", "1", "voucher", "1", "-20.00"]]
        source_file = _write_csv(tmp_path / "payments.csv", HEADER, rows)
    ...
    def test_zero_payment_value_is_accepted(self, tmp_path: Path) -> None:
        rows = [["A", "1", "voucher", "1", "0"]]
        source_file = _write_csv(tmp_path / "payments.csv", HEADER, rows)
    ...
    def test_unexpected_payment_type_is_accepted(self, tmp_path: Path) -> None:
        rows = [["A", "1", "cryptocurrency", "1", "20.00"]]
        source_file = _write_csv(tmp_path / "payments.csv", HEADER, rows)
    ...
    def test_blank_payment_type_is_accepted(self, tmp_path: Path) -> None:
        rows = [["A", "1", "", "1", "20.00"]]
        source_file = _write_csv(tmp_path / "payments.csv", HEADER, rows)
    ...
    def test_zero_payment_installments_is_accepted(self, tmp_path: Path) -> None:
        rows = [["A", "1", "voucher", "0", "20.00"]]
        source_file = _write_csv(tmp_path / "payments.csv", HEADER, rows)
    ...
    def test_negative_payment_installments_is_accepted(self, tmp_path: Path) -> None:
        rows = [["A", "1", "voucher", "-1", "20.00"]]
        source_file = _write_csv(tmp_path / "payments.csv", HEADER, rows)
    ...
    def test_blank_payment_installments_is_accepted(self, tmp_path: Path) -> None:
        rows = [["A", "1", "voucher", "", "20.00"]]
        source_file = _write_csv(tmp_path / "payments.csv", HEADER, rows)
    ...
    def test_duplicate_order_id_values_are_accepted(self, tmp_path: Path) -> None:
        # Expected, everyday shape: order "A" has two payment records.
        connector = _make_connector(tmp_path, _valid_payments_csv(tmp_path))
    ...
    def test_unusual_payment_sequential_value_is_accepted(self, tmp_path: Path) -> None:
        rows = [["A", "99999", "voucher", "1", "20.00"]]
        source_file = _write_csv(tmp_path / "payments.csv", HEADER, rows)
...
class TestCountRecords:
    def test_excludes_header(self, tmp_path: Path) -> None:
        source_file = _write_csv(tmp_path / "payments.csv", HEADER, SAMPLE_ROWS[:1])

        connector = _make_connector(tmp_path, source_file)

        assert connector.count_records() == 1

    def test_multiple_payments_for_same_order_each_counted(self, tmp_path: Path) -> None:
        # Order "A" has two payment rows; both must count as separate
    ...
    def test_header_only_csv_returns_zero(self, tmp_path: Path) -> None:
        source_file = _write_csv(tmp_path / "payments.csv", HEADER, [])
    ...
    def test_quoted_fields_containing_commas_are_handled(self, tmp_path: Path) -> None:
        rows = [["A", "1", "credit_card, installments", "3", "80.00"]]
        source_file = _write_csv(tmp_path / "payments.csv", HEADER, rows)
...
class TestEndToEndLifecycle:
    ...
    def test_record_count_matches_number_of_payment_rows(self, tmp_path: Path) -> None:
        connector = _make_connector(tmp_path, _valid_payments_csv(tmp_path))
    ...
    def test_schema_version_appears_in_completed_metadata(self, tmp_path: Path) -> None:
        connector = _make_connector(tmp_path, _valid_payments_csv(tmp_path), schema_version="9.9")
    ...
    def test_validation_failure_does_not_land_a_raw_file(self, tmp_path: Path) -> None:
        header = [column for column in HEADER if column != "payment_type"]
