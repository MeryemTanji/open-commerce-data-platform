...
- **Transactional** sources (orders, order_items, payments, reviews)
  load into an ingestion-time partitioned table, targeting one explicit
  historical partition via a ``$YYYYMMDD`` decorator, with
...
class BigQueryLoadResult:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.ingestion_date, date):
            raise TypeError("ingestion_date must be a datetime.date")
...
class BigQueryRawLoader:
    ...
    def load(self, source_object: str, gcs_uri: str, ingestion_date: date) -> BigQueryLoadResult:
        ...
        Raises:
            ...
            TypeError: if ``ingestion_date`` is not a ``datetime.date``.
        ...
        if not isinstance(ingestion_date, date):
            raise TypeError("ingestion_date must be a datetime.date")

        is_transactional = source_object in TRANSACTIONAL_SOURCE_OBJECTS
        destination = self._build_destination(source_object, ingestion_date, is_transactional)
    ...
    def _build_destination(self, source_object: str, ingestion_date: date, is_transactional: bool) -> str:
        table = f"{self.project_id}.{self.dataset_id}.{source_object}"
        if is_transactional:
            return f"{table}${ingestion_date.strftime('%Y%m%d')}"
        return table